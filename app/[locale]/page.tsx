            ...
            <section ref={heroRef} className="relative w-full min-h-[95vh] flex flex-col justify-center px-6 md:px-12 lg:px-24 pt-24 pb-12 border-b border-gray-200 overflow-hidden">
                ...
                <motion.div style={{ y: heroY, opacity: heroOpacity }} className="flex flex-col gap-8 max-w-[1400px] mx-auto w-full z-10">
                    ...
                    <FadeIn delay={0.6} className="mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                        ...
                        <div className="flex items-center gap-4 text-sm font-medium uppercase tracking-widest pl-4 sm:pl-8 border-l border-gray-200">
                            {[
                                { name: "AWS", hoverColor: "hover:text-[#FF9900]" },
                                { name: "Azure", hoverColor: "hover:text-[#0089D6]" },
                                { name: "Terraform", hoverColor: "hover:text-[#844FBA]" }
                            ].map((tech, i) => (
                                <motion.div key={tech.name} className="flex items-center" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 + i * 0.1, duration: 0.5 }}>
                                    {i > 0 && <span className="inline-block w-1 h-1 rounded-full bg-gray-300 mr-4" />}
                                    <motion.span
                                        className={`cursor-pointer transition-colors duration-300 text-gray-400 ${tech.hoverColor}`}
                                        animate={{ y: [0, -3, 0] }}
                                        transition={{ duration: 3, repeat: Infinity, repeatType: "reverse", ease: "easeInOut", delay: i * 0.3 }}
                                    >
                                        {tech.name}
                                    </motion.span>
                                </motion.div>
